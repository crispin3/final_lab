...
TrajBuilder::TrajBuilder()  {
    ...
    accel_max_ = 0.2; //0.5; //1m/sec^2
    alpha_max_ = 0.2; //0.2; //1 rad/sec^2
    speed_max_ = 0.2; //1.0; //1 m/sec
    omega_max_ = 0.2; //1.0; //1 rad/sec
...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    nav_msgs::Odometry des_state;
    int npts=vec_of_states.size();
    int a;
     
    for(int i=0;i<npts;i++){
         if(start_pose.pose.position.x==vec_of_states[i].pose.pose.position.x && start_pose.pose.position.y==vec_of_states[i].pose.pose.position.y && start_pose.pose.position.z==vec_of_states[i].pose.pose.position.z)
          {
             a=i; break;
           }
    }//end for
    ...
    ROS_INFO("the status is = %d",a);
    des_state.twist.twist = vec_of_states[a].twist.twist;
    ...
    if (des_state.twist.twist.linear.x!=0&&des_state.twist.twist.angular.z==0){
        cur_vel=des_state.twist.twist.linear.x;
        double t_ramp = cur_vel/accel_max_;
        int npts_ramp = round(t_ramp / dt_);
        double speed_des = 0.0;
        double psi_des = psi_start;
        des_state.twist.twist.angular.z = 0.0; //omega_des; will not change
        ...
        for (int i = 0; i < npts_ramp; i++) {
              speed_des = cur_vel-accel_max_*dt_; //Euler one-step integration
              des_state.twist.twist.linear.x = speed_des;
              x_des += speed_des * dt_ * cos(psi_des); //Euler one-step integration
              y_des += speed_des * dt_ * sin(psi_des); //Euler one-step integration        
              des_state.pose.pose.position.x = x_des;
    ...
    if(des_state.twist.twist.linear.x==0&&des_state.twist.twist.angular.z!=0){
         cur_ori=des_state.twist.twist.angular.z;
         double t_ramp = fabs(cur_ori) / alpha_max_;
         int npts_ramp = round(t_ramp / dt_);
         double psi_des = psi_start; //start from here
         double omega_des = 0.0; // assumes spin starts from rest; 
         // position of des_state will not change; only orientation and twist
         double t = 0.0;
         double accel = sgn(cur_ori) * alpha_max_; //watch out for sign: CW vs CCW rotation
         omega_des=cur_ori;
         for (int i = 0; i < npts_ramp; i++) {              
                  omega_des -= accel*dt_;
                  des_state.twist.twist.angular.z = omega_des;
    ...
    ROS_INFO("break finish");
          

}

void TrajBuilder::build_trapezoidal_backup_traj(geometry_msgs::PoseStamped start_pose, geometry_msgs::PoseStamped end_pose, std::vector<nav_msgs::Odometry> &vec_of_states){
	    double x_start = 3.8;
    double y_start = -1.8;
    double x_end = 3.8;
    double y_end = 0;
    ...
    nav_msgs::Odometry des_state;
    des_state.header = start_pose.header; //really, want to copy the frame_id
    des_state.pose.pose.position.x=3.8;
    des_state.pose.pose.position.y=-1.8;
    des_state.twist.twist = halt_twist_; // insist on starting from rest
    int npts_ramp = round(t_ramp / dt_);
    double x_des = x_start; //start from here
    double y_des = y_start;
    ...
    //make sure the last state is precisely where requested, and at rest:
    des_state.pose.pose.position.x = 3.8;
    des_state.pose.pose.position.y = 0;
    //but final orientation will follow from point-and-go direction
...
void TrajBuilder::build_point_and_go_traj(geometry_msgs::PoseStamped start_pose,
    ...
    if(start_pose.pose.position.x==3.8&&start_pose.pose.position.y==-1.8&&end_pose.pose.position.y==0&&end_pose.pose.position.x==3.8)
    {
		ROS_INFO("building back trajectory");
        build_trapezoidal_backup_traj(start_pose, bridge_pose, vec_of_states);
